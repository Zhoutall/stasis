...
static inline stasis_skiplist_node_t * stasis_util_skiplist_get_lock(
  ...
  stasis_skiplist_node_t * z = hazard_ref(list->h, 2, stasis_util_skiplist_get_forward(x, i));
  while(stasis_util_skiplist_cmp_helper(z, searchKey) < 0) {
...
static inline void * stasis_util_skiplist_insert(stasis_skiplist_t * list, void * searchKey) {
  ...
  for(int i = L; i > 0; i--) {
    ...
    update[i] = hazard_ref(list->h, (L-i)+STASIS_SKIPLIST_HP_COUNT,
        (hazard_ptr*)&x);
  }
  ...
  if(stasis_util_skiplist_cmp_helper(y, searchKey) == 0) {
    ...
    hazard_release(list->h, 1);
    for(int i = L; i > 0; i--) {
      hazard_release(list->h, i+STASIS_SKIPLIST_HP_COUNT);
    }
    return oldKey;
  }
...
static inline void * stasis_util_skiplist_delete(stasis_skiplist_t * list, void * searchKey) {
  ...
  for(i = L+1; i > 1;) {
    ...
    update[i] = hazard_set(list->h, STASIS_SKIPLIST_HP_COUNT+(L-i), (void*)x);
  }
